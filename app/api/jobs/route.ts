...
async function fetchUp2Staff(): Promise<RawJob[]> {
   try {
      ...
      const res = await fetch("https://up2staff.com/jm-ajax/get_listings/", {
         method: "POST",
         headers: {
            "accept": "*/*",
...
export async function GET() {
   ...
   try {
      // Fetch all sources concurrently — 7 sources total
      const [remotive, remoteOK, jobicy, arbeitnow, himalayas, wwr, up2staff] = await Promise.allSettled([
         fetchRemotive(),
         fetchRemoteOK(),
         fetchJobicy(),
         fetchArbeitnow(),
      ...
      const allJobs: RawJob[] = [
         ...
         ...(up2staff.status === "fulfilled" ? up2staff.value : []),
      ]
      ...
      return NextResponse.json({
         ...
         meta: {
            ...
            sources: {
               ...
               up2staff: up2staff.status === "fulfilled" ? up2staff.value.length : 0,
            },
